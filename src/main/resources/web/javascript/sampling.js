 ...
 */
$(document).on("change", "#databaseName", function () {
    loadDatabaseProperties();

    let databaseName = getSelectedDatabase();

    let btn = $('#execute-button');
...
$(document).on("click", "#execute-button", function () {
    ...
    let reqData = {
        ...
        samplingType: $("#samplingType").val(),
        properties: buildPropertiesArray()
    };

    $.ajax({
        url: 'http://localhost:2342/sampling/',
        datatype: "text",
        type: "post",
...
function showCorrectInputs() {
    $('.form-sample-input').hide();

    let value = $("#samplingType").val();
...
$(document).on("change", ".int-input", function () {
    let input = $(this);
...
function drawGraph(data, initial = true) {
    ...
    if (initial) {
        ...
        saveNodePositions(cy.nodes());
    }
    else {
        restoreNodePositions(cy.nodes());
    }
}
